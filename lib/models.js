...
MapBlock = (function (_super) {
  ...
  MapBlock.prototype.previewUrl = function () {
    if (this.source === 'google_maps') {
      return 'https://maps.googleapis.com/maps/api/staticmap?' + 'key=' + GOOGLE_API_CLIENT_KEY + '&center=' + this.escape(this.reference.mapQuery) + '&maptype=' + this.escape(this.reference.mapType) + '&size=' + '520x300&markers=color:red|' + this.escape(this.reference.mapQuery);
    }
  };
...
NewsBlock = (function (_super) {
  ...
  NewsBlock.prototype.thumbnailUrl = function () { // TODO make real and also the ones below
    return this.reference.thumbnailUrl || '//res.cloudinary.com/fold/image/upload/v1/static/LINK_SQUARE.svg';
  };
